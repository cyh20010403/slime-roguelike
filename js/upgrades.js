...
export const ALL_UPGRADES = [
  ...
  { id: 'fire_rate_1', name: '快速射击', desc: '射速 +30%', rarity: 'common', category: 'stat',
    apply() { player.fireRate *= 1.3; }, emoji: '🔫', maxStacks: 8,
    descStack(n) { return `射速 ${(player.fireRate).toFixed(0)} 发/秒`; } },
  { id: 'fire_rate_2', name: '极速射击', desc: '射速 +60%', rarity: 'rare', category: 'stat',
    apply() { player.fireRate *= 1.6; }, emoji: '💨', maxStacks: 5,
    descStack(n) { return `射速 ${(player.fireRate).toFixed(0)} 发/秒`; } },
  { id: 'bullet_count', name: '双管齐下', desc: '子弹数 +1 (扇形散射)', rarity: 'rare', category: 'stat',
    apply() { player.bulletCount = Math.min(player.bulletCount + 1, 9); }, emoji: '🔀', maxStacks: 8,
    descStack(n) { return `子弹数 ${player.bulletCount}`; } },
  { id: 'bullet_dmg', name: '子弹强化', desc: '子弹伤害 +50%', rarity: 'common', category: 'stat',
    apply() { player.bulletDamage += 0.5; }, emoji: '💪', maxStacks: 10,
    descStack(n) { return `子弹伤害 x${player.bulletDamage.toFixed(1)}`; } },
  { id: 'bullet_speed', name: '高速子弹', desc: '子弹速度 +40%', rarity: 'common', category: 'stat',
    apply() { player.bulletSpeed *= 1.4; }, emoji: '🚀', maxStacks: 6,
    descStack(n) { return `子弹速度 ${player.bulletSpeed.toFixed(0)}`; } },
  { id: 'piercing', name: '穿透弹', desc: '子弹穿透敌人', rarity: 'legendary', category: 'effect',
    apply() { player.piercing = (player.piercing || 0) + 1; }, emoji: '🔱', maxStacks: 3,
    descStack(n) { return `穿透 ${n} 个敌人`; } },
  { id: 'explosive', name: '爆炸弹', desc: '子弹命中时爆炸', rarity: 'legendary', category: 'effect',
    apply() { player.explosive = (player.explosive || 0) + 1; }, emoji: '💣', maxStacks: 5,
    descStack(n) { return `爆炸范围 +${n * 30}px`; } },
